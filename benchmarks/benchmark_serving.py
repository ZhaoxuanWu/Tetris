...
def sample_custom_dataset_requests(
    dataset_path: str,
    num_requests: int,
    ...
    dataset = [(apply_chat_template(data["conversations"][0]['value'],
                                    model_name)) for data in dataset]
    ...
    for i in range(len(dataset)):
        ...
        output_len = int((2048 - prompt_len)*0.8)
...
def sample_sharegpt_requests(
    ...
    # Only keep the first two turns of each conversation.
    dataset = [(apply_chat_template(data["conversations"][0]['value'], model_name),
...
def sample_sonnet_requests(
    ...
    base_prompt_formatted = apply_chat_template(base_prompt, model_name)
    ...
    for _ in range(num_requests):
        ...
        prompt_formatted = apply_chat_template(prompt, model_name)
...
def apply_chat_template(prompt: str, model_name: str):
    NAMES_TO_KEYS = {
        ...
        "neuralmagic/Meta-Llama-3.1-405B-Instruct-FP8": "llama-3",
        "hugging-quants/Meta-Llama-3.1-405B-Instruct-AWQ-INT4": "llama-3",
        "neuralmagic/Meta-Llama-3-70B-Instruct-FP8": "llama-3",
    }
...
def main(args: argparse.Namespace):
    ...
    if args.dataset is not None:
        ...
        PATH_TO_SPEC_BENCH = "[PATH_TO_SPEC_BENCH]"
        datasets_to_use_custom_sampler = ['spec_bench', 'domain_tough', 'mix']
        use_custom_sampler = any(substring in args.dataset for substring in datasets_to_use_custom_sampler)
        if use_custom_sampler:
            if args.dataset == "spec_bench_rag_qa_trans":
                input_requests = sample_custom_dataset_requests(
                    dataset_path=f"{PATH_TO_SPEC_BENCH}/rag.json",
                    num_requests=args.num_prompts // 3,
                    tokenizer=tokenizer,
                    fixed_output_len=args.sharegpt_output_len,
                    model_name=args.model)
                input_requests += sample_custom_dataset_requests(
                    dataset_path=f"{PATH_TO_SPEC_BENCH}/qa.json",
                    num_requests=args.num_prompts // 3,
                    tokenizer=tokenizer,
                    fixed_output_len=args.sharegpt_output_len,
                    model_name=args.model)
                input_requests += sample_custom_dataset_requests(
                    dataset_path=f"{PATH_TO_SPEC_BENCH}/translation.json",
                    num_requests=args.num_prompts // 3 + args.num_prompts % 3,
                    tokenizer=tokenizer,
                    fixed_output_len=args.sharegpt_output_len,
                    model_name=args.model)
                # shuffle with seed
                ...
                input_requests = sample_sonnet_requests(
                    dataset_path=f"{PATH_TO_SPEC_BENCH}/sonnets.txt",
                ...
                input_requests += sample_custom_dataset_requests(
                    dataset_path=f"{PATH_TO_SPEC_BENCH}/domain_tough.json",
                    num_requests=args.num_prompts // 2,
                    tokenizer=tokenizer,
    ...
    elif args.dataset_name == "all_mix":
        input_requests = sample_sharegpt_requests(
        ...
        # input_requests += sample_cnn_dailymail_requests(
        #     dataset_path=None,
        #     num_requests=args.num_prompts // 4,
        #     tokenizer=tokenizer,
        #     model_name=args.model,
        #     fixed_output_len=args.cnn_dailymail_output_len,
        # )
        input_requests += sample_sharegpt_requests(
            dataset_path="./Arena.json",
            num_requests=args.num_prompts // 3,
            tokenizer=tokenizer,
            fixed_output_len=args.sharegpt_output_len,
            model_name=args.model,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="sharegpt",
        choices=["sharegpt", "sonnet", "random", "hf", "cnn_dailymail", "all_mix"],
